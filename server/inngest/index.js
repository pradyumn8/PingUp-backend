...
const sendNewConnectionRequestReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await step.run('send-connection-request-mail', async () => {
            ...
            <p>You have a new connection request from ${connection.from_user_id.full_name} - @${connection.from_user_id.username}
...
const deleteStory = inngest.createFunction(
    ...
    async ({ event,step})=>{
        ...
        const in24Hours = new Date(Date.now()+24*60*60*1000)
        await step.sleepUntil('wait-for-24-hours', in24Hours)
        await step.run('delte-story',async()=>{
...
const sendNotifincationOfUnseenMessages = inngest.createFunction(
...
export const functions = [
    ...
    deleteStory,
    sendNotifincationOfUnseenMessages
];