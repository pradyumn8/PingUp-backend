...
const StoryViewer = ({ viewStory, setViewStory }) => {
    ...
    useEffect(()=>{
        ...
        if(viewStory && viewStory.media_type !== 'video'){
            ...
            setInterval(()=>{
                elapsed += setTime;
                setProgess((elapsed / duration) * 100);
            },setTime);
    ...
    const renderContent = () => {
        switch (viewStory.media_type) {
            ...
            case 'video':
                return (
                    <video onEnded={()=>setViewStory(null)} src={viewStory.media_url} className='max-w-full max-h-screen object-contain' controls autoPlay />
                );
            case 'text':
                return (
    ...
    return (
        <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#00000' }}>
            ...
            <div className='absolute top-0 left-0 w-full h-1 bg-gray-700'>
                <div className='h-full bg-white translate-all duration-100 linear' style={{ width: `${progress}%` }}>
                </div>
            </div>
            {/* user Info - Top left */}
